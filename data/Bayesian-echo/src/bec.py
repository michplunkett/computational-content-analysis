...
from numpy import all, bincount, diag, exp, eye, identity, ones, savez, zeros
from numpy.random import dirichlet, gamma, multinomial, poisson, random
from scipy.special import psi as digamma
from slice_sampler import multivariate_slice_sample, slice_sample
...
class BEC(object):
    def __init__(
        ...
        use_prior=False,
        non_diagonal=False,
        time_decay_prior=None,
        influence_prior=None,
        word_concentration_prior=None,
        word_pseudocount_prior=None,
    ):
    ...
    def generate(
        self,
        total_number_of_utterances=100,
        ...
        for message in sorted(messages, key=lambda message: message.get_start_time()):
            ...
            for y in range(self._A):
                excitation_pseudocounts[:, y, :] *= exp(
                    -(t - last_message_time) / self._time_decay[y]
                )
            ...
            message.set_excitation_pseudocounts(
                excitation_pseudocounts[:, x, :], senders=None
            )
    ...
    def import_data(self, V, A, messages, time_to_split=100.0):
        ...
        self._total_message_number = (
            self._training_message_number + self._test_message_number
        )

        print("\nLoaded %d messages in total" % (self._total_message_number))
        print(
            "Training set (before t=%.2f): %d messages (%.2f%%)"
            % (
                self._time_to_split,
        ...
        print(
            "Test set (after t=%.2f) %d messages (%.2f%%)"
            % (
                self._time_to_split,
                self._test_message_number,
                self._test_message_number * 100.0 / self._total_message_number,
            )
        )
    ...
    def update_excitation_pseudocounts(self, senders, recipients):
        ...
        messages = (
            m
            for user, message_list in enumerate(self._messages)
        ...
        for message in sorted(messages, key=lambda message: message.get_start_time()):
            ...
            for y in range(self._A):
                excitation_pseudocounts[:, y, :] *= exp(
                    -(t_start - last_message_time) / self._time_decay[y]
                )
            ...
            if x in recipients:
                message.set_excitation_pseudocounts(
                    excitation_pseudocounts[senders, x, :], senders=senders
                )
    ...
    def set_word_concentration(self, word_concentration, x):
        ...
        """
        assert all(word_concentration > 0) and all(
            word_concentration <= self._word_concentration_upper_bound
        )
    ...
    def set_word_pseudocounts(self, word_pseudocount, x, v):
        ...
        """
        assert all(word_pseudocount >= 0) and all(
            word_pseudocount <= self._word_pseudocount_upper_bound
        ), "%s" % (word_pseudocount)
    ...
    def sample(self, iterations, display=20):
        for _s in range(iterations):
            ...
            if display == 1:
                print("sampling influence matrix...")
            self.sample_influence()
            if display == 1:
                print("sampling word concentration parameters...")
            self.sample_word_concentration()
            if display == 1:
                print("sampling word pseudocounts ...")
    ...
    def log_prob(self):
        ...
        if not self.__ll_cached:
            ...
            self.__log_likelihood = sum(
                message.log_prob()
                for message_list in self._messages
                for message in message_list
                if message.get_end_time() <= self._time_to_split
            )
            # ll of the test set
            self.__log_likelihood_test_set = sum(
                message.log_prob()
                for message_list in self._messages
                for message in message_list
                if message.get_start_time() > self._time_to_split
            )
            #
            if self._use_prior:
                self.__log_prior = 0
                self.__log_prior += log_gamma(
                    self._time_decay, *self._time_decay_prior
                ).sum()
                if self._non_diagonal:
                    # off-diagonal entries
                    self.__log_prior += log_gamma(
                        self._influence[eye(self._A) == 0],
                        *self._influence_prior,
                    ).sum()
                else:
                    self.__log_prior += log_gamma(
                        self._influence, *self._influence_prior
                    ).sum()
                self.__log_prior += log_gamma(
                    self._word_concentration, *self._word_concentration_prior
                ).sum()
                self.__log_prior += log_gamma(
                    self._word_pseudocounts, *self._word_pseudocount_prior
                ).sum()
            else:
                # no prior
                self.__log_prior = 0
...
class Message(object):
    def __init__(
        ...
        assert (
            start_time < end_time and end_time <= T
        ), "Error times: start_time=%s, end_time=%s, T=%s" % (
            start_time,
            end_time,
            T,
        )
    ...
    def get_weighted_excitation_pseudocounts(self):
        ...
        for u in range(0, self._A):
            weighted_excitation_pseudocounts[u, :] = (
                self._influence[u] * self._excitation_pseudocounts[u, :]
            )
    ...
    def log_prob(self):
        if not self.__ll_cached:
            self.__ll = log_polya(
                self._token_type_counts,
                self._concentration * self.base_measure(),
            )
            self.__ll_cached = True
    ...
    def log_prob_pseudocount_grad(self):
        ...
        dirichlet_param = (
            ...
            * unnormalized_base_measure
            / unnormalized_base_measure_sum
        )

        deriv = identity(self._V) * (
            unnormalized_base_measure_sum - unnormalized_base_measure
        ) + (1 - identity(self._V)) * (-unnormalized_base_measure)
        deriv /= unnormalized_base_measure_sum**2

        grad = self._concentration * (
            deriv
            * (
                digamma(self._token_type_counts + dirichlet_param)
                - digamma(dirichlet_param)
            )
        ).sum(axis=1)
    ...
    def __str__(self):
        ...
        s += "\nToken proportions: %s" % (
            self._token_type_counts * 1.0 / self._token_type_counts.sum()
        )
