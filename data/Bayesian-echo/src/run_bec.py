...
from numpy import median, savetxt, zeros
...
def main():
    ...
    time_to_split = allMessages[
        int((1 - test_set_percentage) * len(allMessages)) + 1
    ].get_start_time()
    ...
    allMessages_train_set = (
        msg
        for messageList in bec._messages
        for msg in messageList
        if msg.get_end_time() <= bec._time_to_split
    )
    ...
    allMessages_test_set = (
        ...
        for messageList in bec._messages
        for msg in messageList
        if msg.get_start_time() > bec._time_to_split
    )
    ...
    print(
        "Data imported into bec: V = %d, A = %d, T=%.2f, split time=%.5f"
        % (bec._V, bec._A, T, time_to_split)
    )
    print(
        "Training set: %d messages, test set: %d messages"
        % (len(allMessages_train_set), len(allMessages_test_set))
    )
    ...
    for i, msg in enumerate(allMessages):
        ...
        token_proportions_array[i, :] = (
            msg._token_type_counts * 1.0 / msg._token_type_counts.sum()
        )
    ...
    savetxt(
        resultDirName + "sender_and_time.txt",
        sender_and_time_array,
        header="sender\ttime",
        fmt="%d %.2f",
        comments="",
    )
    savetxt(
        resultDirName + "token_proportions.txt",
    ...
    # get the samples
    (
        log_prior_and_log_likelihood_SAMPLE,
        time_decays_SAMPLE,
        influences_SAMPLE,
        word_concentrations_SAMPLE,
        word_pseudocounts_SAMPLE,
    ) = sample_bec(
        bec,
        display=display,
        sample_number=sample_number,
        burn_in_number=burn_in_number,
    ...
    print(
        "Computing the estimated base measure and excitation pseudocounts from posterior median..."
    )
    ...
    for i, msg in enumerate(allMessages):
        ...
        weighted_excitation_pseudocounts_array[i, :] = msg._influence.dot(
            msg._excitation_pseudocounts
        )

    # write to file
    savetxt(
        resultDirName + "base_measure.txt",
        base_measure_array,
        header=v_header,
        fmt="%.7f",
        comments="",
    ...
    savetxt(
        resultDirName + "weighted_excitation_pseudocounts.txt",
