...
from numpy import array, bincount, linspace, poly1d, polyfit
...
class talkbankXML(object):
    ...
    def __init__(
        ...
        for x in sorted(
            self._all_messages_raw.keys(),
            key=lambda x: len(self._all_messages_raw[x]),
            reverse=True,
        ):
        ...
        if remove_stop_words:
            ...
            print(
                "Processed %d messages: %d non-stop token types appeared, %d stop token types appeared"
                % (
                    self._msg_number,
                    len(self._all_word_count.keys()),
                    len(self._all_stop_word_count.keys()),
                )
            )
        else:
            ...
            print(
                ...
                % (self._msg_number, len(self._all_word_count.keys()))
            )
    ...
    def exportMessages(self, V, maxTime=100.0, display=0):
        ...
        if V > len(self._all_word_count.keys()):
            print(
                "\nWarning: V = %d > %d actual tokens"
                % (V, len(self._all_word_count.keys()))
            )
        ...
        exportedMsgCount = 0
        for token in sorted(
            self._all_word_count, key=self._all_word_count.get, reverse=True
        ):
        ...
        for x in range(self._A):
            msgList = (
                []
            )  # list of messages from sender x, in the form of counts, tokens selected
            startTimeList = []
            endTimeList = []
        ...
        for x in range(self._A):
            ...
            for i, msg in enumerate(listOfMSGLists[x]):
                ...
                messageList.append(
                    Message(
                        sender=x,
                        sender_name=self._participant_list[x],
        ...
        for x in range(self._A):
            print(
                "%d messages from agent %d: %s"
                % (len(allMessages[x]), x, self._participant_list[x])
            )
        ...
        print(
            ...
            sum(
                [
                    msg.get_total_token_counts()
                    for x in range(self._A)
                    for msg in allMessages[x]
                ]
            ),
        )
        print(
            "Total token counts in raw text without restricting to top-V",
            w_count,
        )
        print(
            "Removal rate = %.2f%%"
    ...
    def exportToLDAC(self, allMessages, time_to_split, fileNamePrefix="./"):
        ...
        # equal size split
        test_set_size = len(
            [msg for msg in messages if msg.get_start_time() > time_to_split]
        )
        print(test_set_size)
        ...
        for j in range(number_time_slices - 1):
            number_docs_times[j] = int(
                (len(messages) - test_set_size) * 1.0 / (number_time_slices - 1)
            )
        number_docs_times[0] = (
            len(messages) - test_set_size - sum(number_docs_times[1:-1])
        )
        ...
        print(
            ...
            % (sum(number_docs_times[0:-1]), test_set_size)
        )
    ...
    def allWordCount(self):
        return sorted(
            self._all_word_count.iteritems(),
...
if __name__ == "__main__":
    ...
    talkbankxml_instance = talkbankXML(
        xml_filename,
