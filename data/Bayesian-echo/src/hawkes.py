...
class Hawkes(object):
    ...
    def __init__(self, A=2, T=100.0, non_diagonal=False):
        ...
        print(
            "Hawkes initialized (A=%d, T=%.2f, non_diagonal=%s)"
            % (self._A, self._T, self._non_diagonal)
        )
    ...
    def set_nu(self, nu, x=None, y=None):
        ...
        """
        if all(self._nu[x, y] == nu):
            return  # avoid reevaluating ll when not changed
    ...
    def set_baseline_rate(self, baseline_rate, x=None):
        ...
        if all(self._baseline_rate[x] == baseline_rate):
            return  # avoid reevaluating ll when not changed
        if x is None:
            assert shape(baseline_rate) == (self._A,) and all(
                baseline_rate > 0
            ), "%s" % (baseline_rate)
        else:
    ...
    def simulate(self):
        ...
        """
        assert (
            self._nu_set and self._tau_set and self._baseline_rate_set
        ...
        while currentTime <= self._T:
            ...
            for x in range(self._A):
                ...
                D = (
                    1
                    + log(u1)
                    / (intensities_right[x] - self._baseline_rate[x])
                    / self._tau[x]
                )
                u2 = random()
    ...
    def loadData(self, data, time_to_split=100.0):
        ...
        for x in range(self._A):
            print(
                "-- %d events for agent %d  (train: %4d, test: %4d)"
                % (
                    len(self._data[x]),
                    x + 1,
                    self._train_set_sizes[x],
                    self._test_set_sizes[x],
                )
            )
        ...
        if self._ll_cached:
            ...
            for x in range(self._A):
                ...
                for k, currentTime in enumerate(self._data[x]):
                    ...
                    if k > 0:
                        rate_sum_x *= exp(
                            -(currentTime - self._data[x][k - 1]) / self._tau[x]
                        )
                    ## newly added from the cached data
                    if len(self._newly_added_event_from[x][k]) > 0:
                        corresponding_nu = self._nu[
                            self._newly_added_event_from[x][k], x
                        ]
                        rate_sum_x += sum(
                            corresponding_nu
                            * exp(
    ...
    def optimize_parameters(self, verbose=0):
        ...
        init_guess = concatenate(
            (
                self.get_baseline_rate(),
                self.get_tau(),
                self.get_nu().reshape((1, -1))[0],
            )
        )
        ...
        bounds = []
        for x in range(self._A):
            bounds.append((1e-4, 30))  # baseline
        for x in range(self._A):
            bounds.append((0.11, 20))  # tau
        ...
        def cons_lambdas(j):
            return (
                lambda x: 1
                - sum(x[2 * self._A :].reshape((self._A, self._A))[:, j])
                * x[self._A + j]
                - 1e-6
            )
        ...
        res = minimize(
            ...
            bounds=bounds,
            constraints=cons,
            options={"disp": (verbose > 0), "maxiter": 1000, "ftol": 1.0e-9},
        )
        ...
        assert (
            self.test_matrix_norm_constraint()
        ), "matrix norm constraint violated! %s" % (sum(nu, axis=0) * tau)
        ...
        if verbose > 0:
            ...
            print(
                ...
                "tau",
                self._tau_window_size,
                "nu",
                self._nu_window_size,
            )
            ...
            for y in range(self._A):
                if self._non_diagonal:
                    if x == y:
                        ...
                        continue
                upper_bound_nu = 1.0 / self._tau[y] - (
                    sum(self._nu[:, y]) - self._nu[x, y]
                )
                slice_sample(
                    self._nu[x, y],
                    sample_func_nu,
                    window_size=self._nu_window_size,
                    L_bound=0,
    ...
    def sample_tau_and_nu_revised(self, use_prior=False):
        ...
        for x in range(self._A):
            ...
            for y in range(self._A):
                # TODO: remove the assert
                upper_bound_nu = 1.0 / self._tau[x] - (
                    sum(self._nu[:, x]) - self._nu[y, x]
                )
                ## TODO: remove
                upper_bound_nu = min(upper_bound_nu, 10)
                assert (
                    self._nu[y, x] <= upper_bound_nu
                ), "nu = %f, upper_bound_nu = %f" % (
                    self._nu[y, x],
                    upper_bound_nu,
                )
                # slice sampling with inf stepping out limit
    ...
    def sample_baseline_rate(self, use_prior=False):
        ...
        for x in range(self._A):
            # slice sampling with inf stepping out limit
            slice_sample(
                self._baseline_rate[x],
                sample_func_baseline_rate,
                window_size=self._baseline_rate_window_size,
                L_bound=0,
                step_out_limit=4,
            )
    ...
    def read_hawkes_samples(self, hawkes_sample_dir):
        ...
        baseline_rate_samples = loadtxt(
            hawkes_sample_dir + "SAMPLE-baseline-rate.txt", skiprows=1
        )
        ll_samples = loadtxt(hawkes_sample_dir + "SAMPLE-ll.txt", skiprows=1)
        ll_samples_test_set = loadtxt(
            hawkes_sample_dir + "SAMPLE-ll-test-set.txt", skiprows=1
        )
    ...
    def test_goodness_of_fit(self):
        ...
        for x in range(self._A):
            ...
            residuals = rate_mle[x] * (
                array(self._data[x])[1:] - array(self._data[x])[0:-1]
            )
    ...
    def mle_homogenous_poisson(self):
        ...
        for x in range(self._A):
            rate_mle[x] = (
                len([t for t in self._data[x] if t <= self._time_to_split])
                * 1.0
                / self._time_to_split
            )
    ...
    def posterior_predictive_test_set(self, alpha, beta):
        ...
        for x in range(self._A):
            ...
            pred += (
                -log(delta_t)
                + gammaln(N_T + alpha)
                - gammaln(alpha + N_train)
                + N_test * log(p)
                + (N_train + alpha) * log(1 - p)
            )
...
def unitest_simulation():
    ...
    print(
        "strong stationarity (matrix norm constraint): ",
        hawkes_proc.test_matrix_norm_constraint(),
    )
    ...
    print(
        "ll_train =",
        hawkes_proc.log_likelihood(),
        "ll_test =",
        hawkes_proc.log_likelihood_test_set(),
    )
    ...
    for k in range(B + N):
        if k % 20 == 0:
            print("Sampling %d out of %d ..." % (k, B + N))
        if k >= B:
