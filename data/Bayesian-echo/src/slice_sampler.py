...
def slice_sample(
    ...
    assert (
        L <= x_init and x_init <= R
    ), "L = %s, R = %s, L_bound = %s, R_bound = %s, x_init=%s" % (
        L,
        R,
        L_bound,
        R_bound,
...
def multivariate_slice_sample(
    x_init, ll_func, window_size, L_bound=None, R_bound=None, gradient_func=None
):
    """
    ...
    while True:
        ...
        if ll_func(x.copy()) >= z:
            ...
            if gradient_func is not None:
                ...
                lmask = x < x_init
                L[lmask] = x[lmask]
                rmask = lmask is False
                R[rmask] = x[rmask]


def shrinking_rank_slice_sample(
    x_init,
    ll_func,
    gradient_func,
    crumb_covariance,
    ...
    while True:
        ...
        sample_covariance = 1.0 / inverse_sample_covariance
        unscaled_sample_mean += 1.0 / crumb_covariance * (crumb - x_init)
        ...
        x = x_init + projection(
            np.random.multivariate_normal(
                sample_mean, sample_covariance * np.identity(p)
            )
        )

        if (L_bound is not None and (x <= L_bound).any()) or (
            R_bound is not None and (x >= R_bound.any())
        ):
            ...
            if subspace_columns < p - 1:
                ...
                normed_projected_gradient = projected_gradient / np.sqrt(
