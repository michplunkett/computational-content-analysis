...
sys.path.insert(
    0, "/home/home3/guo/.local/lib/python2.7/site-packages"
)  # dirty hack to use local numpy/scipy packages of latest version

import matplotlib
from numpy import savetxt, shape, zeros
...
def sample_bec(
    bec, display=20, sample_number=500, burn_in_number=500, resultDirName="./"
):
    A = bec._A
    V = bec._V
    ...
    for i in range(0, sample_number + burn_in_number):
        ...
        if display == 1:
            print("Sampling influence...")
        influence = bec.sample_influence()
        if display == 1:
            print("Sampling concentration...")
        concentration = bec.sample_word_concentration()
        if display == 1:
            print("Sampling pseudocounts...")
        for _l in range(10):
        ...
        if i >= burn_in_number:
            ...
            log_prior_and_log_likelihood[i - burn_in_number, :] = [
                bec.log_prior(),
                bec.log_likelihood(),
                bec.log_likelihood_test_set(),
            ]
    ...
    savetxt(
        outputfilename_logprior_and_loglikelihood,
        log_prior_and_log_likelihood,
        header="log.prior\tlog.likelihood\tlog.likelihood.test.set",
        comments="",
    )
    # time decay
    ...
    savetxt(
        ...
        time_decays,
        header=timeDecayHeader,
        comments="",
    )
    # word concentrations
    wordCocentrationHeader = "".join(
        ["word.concentration.%d\t" % (i) for i in range(1, A + 1)]
    )
    savetxt(
        outputfilename_word_concentration,
        word_concentrations,
        comments="",
        header=wordCocentrationHeader,
    )
    # influences
    influenceHeaders = (
        "%s\t"
    ...
    savetxt(
        outputfilename_influence,
        influences.reshape((shape(influences)[0], A * A)),
        header=influenceHeaders,
        comments="",
    )
    # word pseudocounts
    wordPseudocountsHeader = "".join(
        [
            "word.pseudocount.%d.for.word.%d\t" % (i, j)
            for i in range(1, A + 1)
            for j in range(1, V + 1)
        ]
    )
    savetxt(
        outputfilename_pseudocounts,
        word_pseudocounts.reshape((shape(word_pseudocounts)[0], A * V)),
        comments="",
        header=wordPseudocountsHeader,
    )
